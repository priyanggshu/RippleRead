...
export const fetchBlogsController = async (req, res) => {
  try {
    ...
    const cacheKey = `blogs-page-${page}-limit-${limit}-search-${
      search || "all"
    }`;
...
export const likeBlogController = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Blog like status updated", likes: blog.likes.length });
  } catch (error) {
    res.status(500).json({ message: "Server error" });
    console.error(error.message);
  }
...
export const getRecommendationsController = async (req, res) => {
  try {
    ...
    const likedBlogs = await Blog.find({ likes: userId }).select(
      "_id category author"
    );
    ...
    const authors = [
      ...new Set(likedBlogs.map((blog) => blog.author.toString())),
    ];

    const recommendations = await Blog.find({
      $or: [{ category: { $in: categories } }, { author: { $in: authors } }],
...
export const fetchExternalSourcesController = async (req, res) => {
  try {
    ...
    console.log(`data from external: ${data} `)
